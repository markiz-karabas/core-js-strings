...
function unbracketTag(str) {
  const tegOpen = /</;
  const tegClose = />/;
  let formatStr = str.replace(tegOpen, '');
  formatStr = formatStr.replace(tegClose, '');
  return formatStr;
}
...
function encodeToRot13(str) {
  const originAlpha = Array.from(
    'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'
  );
  const ROT13 = Array.from(
    'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm'
  );
  const regExpOfReng = /[a-z]/gi;
  const strEncode = str.replace(regExpOfReng, (char) => {
    const index = originAlpha.indexOf(char);
